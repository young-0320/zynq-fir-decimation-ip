...
- 방법: Clocking Wizard(MMCM)로 PL 전체를 목표 주파수로 정확히 분주한다 (요청 = 실제, 오차 0).
...
115 MHz로 낮춰 마진을 확보한다. bit/xsa: `build/fir_n43_v1_freq_115mhz/output/`
...
| ---------- | ------ | ----------- | ---- | ----- | ------------- | ---- |
| 115.000    | +0.231 | true        | 4582 | 16    | 1.699         | 수정 전 골든 행과 전 수치 동일 |

- 크리티컬 패스 불변: `u_fir_n43/prod_reg_reg → round_reg_reg` (FIR 코어 CARRY4 체인,
  logic 21 levels) — 래퍼 경로 아님. 래퍼 수정(hold-back FF 3개 + 소수 LUT)은 타이밍 중립.
- 판정: Fmax 116 MHz 특성은 수정 후 넷리스트에서도 유효하다. 재스윕 불필요.
  (조건 분기: 크리티컬 패스가 래퍼로 이동했거나 WNS 마진이 유의미하게 줄었다면 경계 부근
  국소 재스윕 — 해당 없음으로 종결)
- correctness: v1@115MHz 보드 기능 동작 확인 완료 (SD boot + `mode 1-1` 데모 정상,
  workflow_v22 §3). Fmax 정밀 측정(오실로스코프 등)은 범위 제외.

---

## 보조지표 교차 참조 (§8, 2026-07-22 통합)

- ASIC 합성 대비 (Track A): v1/v2 동일 제약 sweep — v1↔v2 격차는 FPGA CARRY4 종속,
  표준셀 합성에서는 동률. `docs/report/fir_n43/summary/asic_vs_fpga.md`
- 보드 전력 실측 (Track B): 5V 입력 실측 S1 2.21W, Vivado 추정 1.705W와 정합 범위
  (USB-UART 0.22W 실측 분리). `docs/report/fir_n43/summary/power_board_vs_vivado.md`
