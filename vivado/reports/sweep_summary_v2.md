...
타이밍 마진은 약 6배이므로, 실보드 배포 주파수로 145 MHz를 택한다.
...
- worst 경로: `waiting_for_last_out_reg → u_fir_n43_v2/prod_comb(DSP48.A)` — 래퍼 FSM에서
  ...
  동일하다(+0.129). 위 "크리티컬 패스" 절이 기록했듯 145+ 경계에서는 DMA IP/래퍼 인터페이스
  경로가 원래 비슷하게 임계였다 — hold-back이 새로 만든 경로가 아니다.
- 판정: Fmax 146 MHz 특성은 수정 후 넷리스트에서도 유효하다. 재스윕 불필요.
