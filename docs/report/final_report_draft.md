...
> Track B 전력 실측 완료(2026-07-22) 반영 — 플레이스홀더 없음. 수치는 전부 리포 문서에서 검증된 값.
...
### 5.2 전력 실측 (§8-3) — 완료 (2026-07-22)

- 방법: 보드 5V 배럴잭 입력 직렬 DMM(HDS242, 10A 잭) + 상태 차분, 상태당 전류 5회
  판독 중앙값, 전압은 부하 상태에서 상태별 실측. 전 상태 USB-UART 연결 기준.
  상세: log 46 §5, `docs/report/fir_n43/summary/power_board_vs_vivado.md`.

| 상태 | 조건 | I_med | V | P |
| --- | --- | ---: | ---: | ---: |
| S0 | SD 미삽입, PL 미구성 | 0.340 A | 5.047 V | **1.72 W** |
| S1 | SD boot, `READY FIR` idle | 0.443 A | 4.988 V | **2.21 W** |
| S2 | `mode 1-1` 반복 실행 중 | 0.435 A | 5.008 V | **2.18 W** |

- **S1−S0 = 0.49W** (PL 구성+145MHz 클럭+앱+DDR 활성 합), **S2−S1 ≈ 0** — 예측대로
  µs 스케일 burst는 평균 전력에 미검출, 이 자체를 결과로 확정.
- **Vivado 1.705W 대비**: 실측 2.21W에서 USB-UART 0.22W(실측 분리) 제외 → 1.99W,
  레귤레이터 효율 85~90% 가정 시 1.69~1.79W로 **추정과 정합 범위**.
- 부가: USB-UART 연결 비용 0.22W 실측 분리(관측 인프라 > 연산 몫), USB 유무 양 조건에서
  S1−S0 델타 재현(0.105/0.103A) — 차분 방법론 자체 검증.
...
- run이 타임아웃으로 **abort된 뒤의 복구는 불가** (펌웨어에 PL 래퍼 리셋 수단 없음) —
  정상 반복 실행 경로는 v22 수정으로 해소(시뮬 검증 + 전력 실측 중 `mode 1-1` reset
  없이 반복 실행 정상 확인, 2026-07-22).
