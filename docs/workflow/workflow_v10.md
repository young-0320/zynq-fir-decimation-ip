...
- 2026-05-07 업데이트: Step 5~6 완료, Step 7/8 코드 완성 — 보드 검증 대기 상태로 갱신. 다음 단계는 `workflow_v11.md` 참고
...
> **2026-05-07 기준:** Step 5~6 완료, Step 7/8 코드 완성. 이하 내용은 Step 5 설계 결정사항 기록이며 현재 모두 완료됨. 보드 검증 절차는 `workflow_v11.md` 참고.

**Step 5 — AXI-Stream 래퍼 구현 ✅ (완료)**
...
- [X] Block Design DRC PASS
- [X] 비트스트림 생성 완료 (WNS=+1.239ns), XSA 생성 완료

---

### Step 7 — bare-metal C + UART 🔄 (코드 완성, 보드 검증 대기)

`sw/fir_decimator_demo.c` 완성. Vitis 빌드 → 보드 다운로드 → UART 동작 확인 필요.

완료 기준:

- [X] `sw/fir_decimator_demo.c` 구현 완료
- [X] `vitis/build_fir_decimator_demo.tcl` / `.py` 빌드 스크립트 완성 (경로 버그 수정 완료)
- [ ] Vitis 빌드 → ELF 생성
- [ ] 보드에서 UART 응답 확인

---

### Step 8 — PC Python FFT 실시간 시각화 🔄 (코드 완성, 보드 검증 대기)

`sw/fir_decimator_demo.py` 완성. 보드 연결 후 실제 데이터로 동작 확인 필요.

완료 기준:

- [X] `sw/fir_decimator_demo.py` 구현 완료 (mode 0 보드 불필요, mode 1/2 보드 필요)
- [ ] 실시간 스펙트럼 플롯 동작 (실보드 데이터)
- [ ] 필터 전/후 비교 시각적으로 명확
...
| **M4 (안전 마감선)** | **6월 말** | **AXI-Stream 래퍼 + BRAM 오프라인 FFT 확인** | 🔄 코드 완성, 보드 검증 대기 |
| M5                         | 7월 2주차        | PS-PL DMA 연동 완료                                | 🔄 Step 6 하드웨어 완료, 보드 검증 대기 |
...
└── transposed_form/
    └── n43/
        ...
        └── fir_decimator_transposed_n43_axis_top.v  ✅ Step 5
...
└── transposed_form/
    ...
    └── tb_fir_decimator_transposed_n43_axis_top.v  ✅ Step 5

sw/
├── fir_decimator_demo.c    ✅ Step 7 코드 완성
└── fir_decimator_demo.py   ✅ Step 8 코드 완성

vivado/
├── build_fir_transposed_n43.tcl  ✅ Step 4/6 Vivado 빌드
└── bd_fir_dma.tcl                ✅ Step 6 Block Design

vitis/
├── build_fir_decimator_demo.tcl  ✅ Step 7 빌드 스크립트
└── build_fir_decimator_demo.py   ✅ Step 7 빌드 스크립트 (Python API)

docs/log/
├── ...
├── 17_axi_stream_wrapper_design_decisions.md  ✅ Step 5 설계 결정
├── 19_ps_pl_dma_integration_design.md         ✅ Step 6 설계 결정
├── 20_baremetal_c_fir_dma.md                  ✅ Step 7 설계 결정
├── 21_vitis_build_and_uart_usage.md           ✅ Step 7 빌드/사용법
├── 22_pc_python_fft_visualization_plan.md     ✅ Step 8 설계 결정
└── workflow_v10.md                            ← 현재 문서 (보드 검증 전)
```
