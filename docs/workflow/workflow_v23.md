...
- Purpose: log38 §8 교수님 미팅 피드백(2026-07-02, 보조 지표 4건)을 반영하고 v1.0 문서를
  마무리하기 위해 **다음에 해야 할 작업들을 정리**한다. **이 문서는 계획(할 일 목록)만
  다룬다 — 착수/실행은 아직 아님. 수치·결과는 이후 log로.**
...
목표: `docs/report/fir_n43/plot/cpu_vs_fpga_timing_{window,ubuntu}.png`에 CPU 스펙 +
측정 방법론 텍스트 기입.

- [사용자 정보 제공, 착수 전 확인 필요] **C1 CPU 스펙**: Windows / Ubuntu 각 머신의
  CPU 모델명·코어수·클럭. (물리 조작 아님 — 각 머신에서 스펙만 확인해 알려주면 됨.)
- [에이전트] `sw/cpu_benchmark.py` 수정(방법론 텍스트는 log38 §0-2에 이미 정리됨: numpy
  float64 `np.convolve`, `perf_counter`, convolve 구간만, UART 제외) → PNG 2장 재생성.
...
- **v22와 무관해 착수 가능한 트랙**: Track C(CPU 스펙 받으면), Track A 서버 실행(대상 RTL
...
4. **C1** Windows·Ubuntu CPU 스펙(모델/코어/클럭).
...
| 순서 | 작업 | 주체 | 상태/의존 |
|---|---|---|---|
| C | CPU 벤치 PNG에 스펙·방법론 텍스트 (§8-2) | 사용자(스펙 제공) → 에이전트(구현) | CPU 스펙 받으면 즉시 |
...
| Fmax검증 | v1/v2 BOOT.bin 재생성 + 보드 실측 (wf22 로드맵 §3·§4) | 사용자(보드 실행) → 에이전트(반영) | v22 완료 후 |
