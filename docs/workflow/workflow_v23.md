...
- [에이전트] 결과 정리: 실측 전력 표 + Vivado 추정 대비 오차 분석 (결과는 log 46 §5에 기록).
...
- ❌ (data-blocked) 위 Track A~C 결과를 v1.0 문서에 **보조 지표로 통합** (§8-4.5) — A(ASIC)/
  B(전력) 실측 데이터 도착 후에만 가능. C(CPU PNG 주석)는 완료.
...
| D2       | §5.1 한계 해소 표시 + A/B/C 결과 v1.0 문서 통합 (§8-4.5) | 에이전트                                  | §5.1 표시·C 완료. A/B 통합은 실측 데이터 도착 후                       |
