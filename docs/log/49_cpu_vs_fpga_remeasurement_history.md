...
## CPU 값 요동 관찰 (2026-07-22 추가)

동일 노트북(Windows, i5-1340P)에서 스크립트를 연속 실행한 세션별 CPU median 판독:

```text
181.2 / 223 / 208.8 / 174.4 / 174.8 / 175.9 (µs)
추가 관찰 범위: 최소 ~170µs, 최대 ~300µs
```

- 세션 간 요동 원인: OS 스케줄링·전원 상태·백그라운드 부하 (스크립트 내부의 CV<2%
  수렴은 세션 내 분산만 통제하고, 세션 간 분산은 통제하지 않음).
- 같은 기간 FPGA 값은 전 세션 85µs 고정 — 결정론적 처리 시간 주장의 직접 증거.
- **보고서 확정값은 221.0/85.0으로 픽스** (최종 차트 기준). CPU 요동 범위(약 170~300µs)는
  보고서에서 결정론 논거의 보조 수치로 인용 가능.

## 차트 생성 시각 표기 (2026-07-22 추가)

`cpu_benchmark.py` 차트 출력에 생성 시각을 추가:

- 이미지 우상단에 `generated YYYY-MM-DD HH:MM` 회색 소자 표기
- 기본 출력 파일명에 타임스탬프 포함: `cpu_vs_fpga_timing_<YYYYMMDD_HHMMSS>.png`
  (`--chart-out` 명시 시 그대로 사용). 기존 `cpu_vs_fpga_timing.png`(221/85)는
  타임스탬프 기능 이전 산출물.
