...
5. decimator는 `phase` 파라미터를 유지하되 기본값은 `0`으로 두고, `m=2` 기준 `0, 2, 4, ...` 인덱스를 선택하는 정책으로 확정했다.
...
### 4. `phase=0`를 기본값으로 둔 근거

- decimator의 핵심 동작은 `x[phase::m]`이며, `phase`는 어떤 샘플부터 살릴지 정하는 오프셋임.
- 현재 프로젝트의 기본 디시메이션 계수는 `M=2`이므로:
  - `phase=0` -> `0, 2, 4, ...` 인덱스 선택
  - `phase=1` -> `1, 3, 5, ...` 인덱스 선택
- 기본값을 `0`으로 두면 첫 FIR 출력 샘플부터 바로 추적할 수 있어 디버깅과 baseline 비교가 가장 단순함.
- `phase` 자체는 열어두어, 이후 RTL valid 정렬이나 비교 실험에서 샘플 선택 시작점을 바꿔 확인할 수 있도록 함.
