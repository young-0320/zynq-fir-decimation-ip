...
2. quantization, rounding, saturation, overflow 처리 정책이 문서에 명시되어 있음.
3. fixed 모델 단독 동작과 예외 처리가 테스트로 검증되어 있음.
4. `sim/python/test/fixed` 기준 테스트가 통과함.
5. 이후 ideal-vs-fixed 비교 단계로 넘어가기 위한 고정소수점 기준 모델이 정리되어 있음.

## 2) 주간 루틴

1. 월-화: fixed-point 포맷/연산 정책 정리 및 구현.
2. 수-목: quantization/rounding/saturation 정책별 단위 테스트와 bit-level 동작 점검.
...
4. ideal-vs-fixed 비교 및 정량 검증 단계부터는 `workflow_v3.md`를 기준 문서로 사용한다.
