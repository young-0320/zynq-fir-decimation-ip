...
### 8-6. 단일 mwr 워드 주입 방식 시도 — 실패

`download_and_run.py`로 ELF 전체를 워드 단위 단일 `mwr`(pexpect REPL barrier)로 주입 완료(13,769 words).

완료 후 수동 `mrd -force 0x100000 4` 확인:
...
**pexpect REPL barrier 방식에서도 byte[3] 오염 재발.** 24 문서가 "검증된 방법"으로 기록했으나, 실제로는 비결정적으로 실패함. → Phase 9 참조.
...
모든 접근이 실패로 종료됨. 최종 결론 → Phase 9 / 최종 결론 섹션 참조.

---

## Phase 9: 최종 검증 시도 및 실패 확인 (2026-05-13)

### dow 반복 테스트

`dow` 명령을 여러 차례 실행하며 벡터 테이블 readback:

| 실행 | 0x100000 | 0x100004 | 0x100008 | 0x10000C |
|---|---|---|---|---|
| 1차 (word-by-word 완료 후) | F9000031 | F900000D | F9000013 | **EA**000023 ✅ |
| 2차 (dow) | F2000031 | F200000D | F2000013 | **EA**000023 ✅ |
| 3차 (dow) | F2000031 | F200000D | **F4**000013 | F2000023 ❌ |

byte[3] 오염값이 F9, F2, F4로 매 실행마다 달라 **비결정적(non-deterministic)** 확정.  
"마지막 워드만 정상" 패턴도 3차에서 붕괴(0x10000C도 오염).

### 패치 후 실행 시도

2차 dow 후 0x100008을 수동 패치(`mwr -force 0x100008 0xEA000013`) 후 `con`:

```
rrd pc → 0x00000010  (Data Abort 벡터, OCM 기준)
rrd   → SP=0x00000000, LR=0x00000014
```

- SP=0: abort mode 스택이 초기화되기 전에 예외 발생 → Data Abort 핸들러가 재진입하며 무한 루프
- LR=0x14: 예외 발생 명령어 주소 = LR-8 = 0x0C (OCM 예외 벡터 영역) → 벡터 테이블 미설정 상태에서 예외 발생

entry point 근처 확인:
```
mrd -force 0x100B04 1 → EBA00000
```

0x100B04 byte[3] = 0xEB (BL opcode), 기대값은 데이터 처리 명령어(0xE1 계열). 벡터 테이블 패치만으로는 해결 불가 — ELF 곳곳이 오염됨.

---

## 최종 결론 — JTAG ELF 로딩 미해결, SD카드 부팅 전환

### 시도한 모든 방법 요약

| 방법 | 결과 |
|---|---|
| `source load_elf.tcl` (mwr × 13769) | byte[3] 오염 (sourced 모드, REPL barrier 없음) |
| `source load_elf.tcl` + mwr+mrd 쌍 | byte[3] 여전히 오염 (sourced 모드는 barrier 재현 불가) |
| `source load_elf.tcl` + `after 1` | byte[3] → 0x00으로 null flush |
| `jtag frequency 1000000` + `dow` | 전송 속도만 감소, byte[3] 오염 동일 |
| `dow` (기본) | byte[3] 비결정적 오염 (F9 / F2 / F4 등) |
| pexpect REPL barrier (단일 mwr × 13769) | byte[3] 비결정적 오염 재발 (Phase 8-6) |
| 레퍼런스 ps7_init.tcl 적용 | 오염값 패턴만 변화, byte[3] 오염 미해결 |
| Board Delay 수정 비트스트림 (Phase 5) | byte[3] 오염 동일 |

### 미해결 근본 원인

두 가설이 공존하며 완전히 배제되지 않음:

1. **xsdb JTAG AXI 마스터 파이프라인** — byte[3]의 DDR commit이 명령 경계와 어긋남. pexpect REPL barrier로도 완전 해소되지 않으며, sourced 모드에서는 어떤 방법으로도 재현 불가.
2. **DDR PHY 타이밍** — DQ[31:24](byte lane 3)의 write leveling이 이 보드의 실제 회로와 맞지 않아 고속 연속 쓰기에서 timing violation 발생. Board Delay 수정이 오염값 패턴을 바꿨으나 제거하지 못했다는 점에서 가설 유효.

두 원인이 복합 작용할 가능성 높음. OCM은 완전 정상 → 문제는 DDR 경로에 국한됨.

### 결정: SD카드 부팅으로 전환

JTAG 직접 부팅의 byte[3] 문제는 xsdb 툴 레이어 + DDR PHY 하드웨어 레이어의 복합 원인으로 추정되며, 현재 환경에서 확정·수정이 불가능하다.

SD카드 부팅으로 전환:
- FSBL(First Stage Boot Loader)이 DDR을 하드웨어적으로 초기화하고 ELF를 로드
- BOOT.BIN(비트스트림 + FSBL + ELF)을 SD카드에 적재 → 보드 부팅 시 자동 실행
