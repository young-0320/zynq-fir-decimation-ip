...
report_power -total_only > "v1_20000ps_power.rpt"
```

power에 `-total_only`를 쓰는 이유: 우리 top은 래퍼 없이 코어 직접 합성이라 top 아래가
곧 leaf 셀 수만 개 — 기본 `report_power`는 인스턴스 4096개에서 잘리며 TOTAL 행까지
생략된다([POWER-155]). `-total_only`는 한 줄에 internal/switching/leakage/total 분해를
모두 출력하므로(PF-263) 이것만으로 충분하다.

파일명 규칙: `{v1|v2}_<period>ps_*`. 산출물은 `results/<ver>/` 아래 평면 보관:

```text
asic/oasys/results/v1/
├── v1_20000ps_timing.rpt
├── v1_20000ps_area.rpt
├── v1_20000ps_power.rpt
└── v1_<period>ps_synth.v   (P&R 입력용 지점만 보관 — 10000/12000ps)
```

실제 sweep 실행 결과와 해석(20000→6000ps, 전 지점 pass, 중단 논리)은
`results/sweep_report.md`에 있다.
