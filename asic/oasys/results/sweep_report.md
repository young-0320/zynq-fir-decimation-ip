...
- 잠정 Fmax 추정: v1 ≈ 92MHz, v2 ≈ 97MHz — 15000ps에서 5배였던 slack 격차가 좁혀짐.
  **두 버전 공통으로 16×16 곱셈→prod_reg 경로(~10ns)가 하한**이라, 라운딩 병합 경로가
  제거된 v2도 이 하한 아래로는 못 내려간다. 다음 지점(10000ps, required 9719.7ps)은
  곱셈 경로가 재구조화되지 않는 한 **둘 다 FAIL 가능성** — v1은 라운딩 경로도 함께
  넘어야 해서 더 불리. 최종 Fmax 차이는 "5배"가 아니라 몇 % 수준으로 수렴할 전망.
