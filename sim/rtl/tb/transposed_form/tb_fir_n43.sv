...
module tb_fir_n43;
  ...
  always @(posedge clk) begin
    if (rst) begin
      ...
      if (timeout_cnt > 1000) begin
        $display("FATAL tb_fir_n43: watchdog timeout — pipeline stalled.");
        $fatal(1);
      end
    end
  end
    ...
    if (!rst) begin
      ...
      if (out_valid) begin
        if (expected_q.size() == 0) begin
          $display("FAIL tb_fir_n43: extra output at cycle=%0d actual=%0d",
                   cycle_cnt, $signed(out_sample));
          $fatal(1);
        end
        exp_val = expected_q.pop_front();
        if ($signed(out_sample) !== $signed(exp_val)) begin
          $display("FAIL tb_fir_n43: idx=%0d cycle=%0d actual=%0d expected=%0d",
  ...
  task automatic drain_and_check();
    ...
    if (expected_q.size() != 0) begin
      $display("FAIL tb_fir_n43: missing %0d outputs after %0d drain cycles.",
               expected_q.size(), d);
      $fatal(1);
    end
  endtask
