...
// tb_fir_n43.sv 사본 — DUT만 fir_n43_v2로 교체 (workflow_v21 Stage A)
// 벡터/비교 로직은 원본과 동일. out_valid 펄스 기준 큐 비교라 latency 변경(3→4 cycle)에
// 영향받지 않으며, 출력값은 v1과 100% 동일해야 한다.

module tb_fir_n43_v2;

  localparam integer CLK_HALF  = 5;
  localparam integer INPUT_LEN = 8192;
  localparam integer FLUSH_LEN = 42;
  localparam integer EXP_LEN   = 8234;
  ...
  always @(posedge clk) begin
    if (rst) begin
      ...
      if (timeout_cnt > 1000) begin
        $display("FATAL tb_fir_n43_v2: watchdog timeout — pipeline stalled.");
        $fatal(1);
      end
    end
  end
    ...
    if (!rst) begin
      ...
      if (out_valid) begin
        if (expected_q.size() == 0) begin
          $display("FAIL tb_fir_n43_v2: extra output at cycle=%0d actual=%0d",
                   cycle_cnt, $signed(out_sample));
          $fatal(1);
        end
        exp_val = expected_q.pop_front();
        if ($signed(out_sample) !== $signed(exp_val)) begin
          $display("FAIL tb_fir_n43_v2: idx=%0d cycle=%0d actual=%0d expected=%0d",
  ...
  // negedge 앵커: DUT는 posedge 샘플 — negedge에서 드라이브해 셋업 타임 확보
  ...
  task automatic drain_and_check();
    ...
    if (expected_q.size() != 0) begin
      $display("FAIL tb_fir_n43_v2: missing %0d outputs after %0d drain cycles.",
               expected_q.size(), d);
      $fatal(1);
    end
  endtask
