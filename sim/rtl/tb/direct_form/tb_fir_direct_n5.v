...
module tb_fir_direct_n5;
        ...
        if (!rst) begin
            ...
            if (out_valid) begin
                if (observed_count >= EXPECTED_LEN) begin
                    $display("FAIL tb_fir_direct_n5: extra output at cycle=%0d actual=%0d (0x%04h)", cycle_count, out_sample, out_sample);
                    $fatal(1);
                end
                ...
                if (out_sample !== expected_sample) begin
                    $display(
                        "FAIL tb_fir_direct_n5: sample_idx=%0d cycle=%0d actual=%0d (0x%04h) expected=%0d (0x%04h)",
                        observed_count,
                        cycle_count,
                        out_sample,
                        out_sample,
    ...
    initial begin
        ...
        $readmemh("sim/vectors/direct_form/bringup_n5/expected_fir_q15.hex", expected_mem);
        ...
        if (observed_count != EXPECTED_LEN) begin
            $display(
                "FAIL tb_fir_direct_n5: expected %0d samples but observed %0d after %0d drain cycles",
                EXPECTED_LEN,
                observed_count,
                drain_cycles
            );
        ...
        $display("PASS tb_fir_direct_n5: observed %0d samples", observed_count);
        $finish;
    end
