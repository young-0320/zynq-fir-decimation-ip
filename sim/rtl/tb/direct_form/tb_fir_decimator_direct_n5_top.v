...
module tb_fir_decimator_direct_n5_top;

    localparam integer CLK_HALF_PERIOD_NS = 5;
    localparam integer INPUT_LEN          = 8192;
    localparam integer FLUSH_LEN          = 4;
    localparam integer EXPECTED_LEN       = 4098;
    ...
    fir_decimator_direct_n5_top dut (
        .clk(clk),
        .rst(rst),
        .in_valid(in_valid),
        .in_sample(in_sample),
        ...
        if (!rst) begin
            ...
            if (out_valid) begin
                if (observed_count >= EXPECTED_LEN) begin
                    $display("FAIL tb_fir_decimator_direct_n5_top: extra output at cycle=%0d actual=%0d (0x%04h)", cycle_count, out_sample, out_sample);
                    $fatal(1);
                end
                ...
                if (out_sample !== expected_sample) begin
                    $display(
                        "FAIL tb_fir_decimator_direct_n5_top: sample_idx=%0d cycle=%0d actual=%0d (0x%04h) expected=%0d (0x%04h)",
                        observed_count,
                        cycle_count,
                        out_sample,
                        out_sample,
    ...
    initial begin
        $readmemh("sim/vectors/direct_form/bringup_n5/input_q15.hex", input_mem);
        $readmemh("sim/vectors/direct_form/bringup_n5/expected_decim_q15.hex", expected_mem);
        ...
        if (observed_count != EXPECTED_LEN) begin
            $display(
                "FAIL tb_fir_decimator_direct_n5_top: expected %0d samples but observed %0d after %0d drain cycles",
                EXPECTED_LEN,
                observed_count,
                drain_cycles
            );
        ...
        $display("PASS tb_fir_decimator_direct_n5_top: observed %0d samples", observed_count);
        $finish;
    end
