...
module fir_decimator_transposed_n43_axis_top #(
    ...
    always @(posedge aclk) begin
        if (!aresetn) begin
            valid0 <= 1'b0; data0 <= 16'sd0;
            valid1 <= 1'b0; data1 <= 16'sd0;
            valid2 <= 1'b0; data2 <= 16'sd0;
        end else begin
            if (transfer) begin
                // shift: reg1→reg0, reg2→reg1, reg2 비움
                valid0 <= valid1;       data0 <= data1;
                valid1 <= valid2;       data1 <= data2;
                valid2 <= 1'b0;
                // 동시 로드: NBA "last wins" — 쉬프트 전 valid1/valid2로 슬롯 결정
                if (core_out_valid) begin
                    if      (!valid1) begin valid0 <= 1'b1; data0 <= core_out_sample; end
                    else if (!valid2) begin valid1 <= 1'b1; data1 <= core_out_sample; end
                    else              begin valid2 <= 1'b1; data2 <= core_out_sample; end
                end
            end else if (core_out_valid) begin
                if      (!valid0) begin valid0 <= 1'b1; data0 <= core_out_sample; end
                else if (!valid1) begin valid1 <= 1'b1; data1 <= core_out_sample; end
                else if (!valid2) begin valid2 <= 1'b1; data2 <= core_out_sample; end
            end
        end
    end
    ...
    always @(posedge aclk) begin
        if (!aresetn)
            ...
            tlast_cnt <= (tlast_cnt == TLAST_N - 1) ? 0 : tlast_cnt + 1;
    end
