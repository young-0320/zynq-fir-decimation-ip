...
// FIR Transposed Form N=43 — v2 (120MHz 크리티컬 패스 보완, workflow_v21 참고)
//
// 설계 기준: docs/log/14_transposed_form_rtl_decisions.md
// v1(fir_n43.v) 대비 변경점: docs/workflow/workflow_v21.md
//
// v1은 Stage 2에서 누산과 round를 같은 사이클에 묶어 계산했다
// (round_reg <= round_q2_30_to_q1_15(prod_reg[0] + z[1])).
// 이 경로가 120MHz 타이밍 위반의 크리티컬 패스였다
// (vivado/reports/sweep_summary.md 참고, CARRY4x19 체인).
//
// v2는 누산(Stage 2)과 round(Stage 3)를 분리해 한 사이클 더 쓴다.
// round는 재계산 없이 레지스터에 저장된 z[0]을 그대로 읽는다.
//
// 파이프라인 4단계:
//   Stage 1: in_valid=1 → h[k]*in_sample → prod_reg[k]  (k=0..42 병렬)
//   Stage 2: prod_reg[k] + z[k+1] → z[k]  (누산만)
//   Stage 3: round(z[0]) → round_reg
//   Stage 4: saturate(round_reg) → out_sample
//
// Latency: 4 cycles (accepted input 기준, v1 대비 +1 cycle)
// z[k]   : signed 48-bit (Q2.30)
// 반올림 : ties-away-from-zero, 출력 1회
// 포화   : 출력 1회 clip(-32768, 32767)
// z[42]  : prod_reg[42] only (z[43] 없음, B안)
// 출력값은 v1과 100% 동일 (계산 순서만 재배치, 로직 자체는 변경 없음)
...
module fir_n43_v2 (
  ...
  always @(posedge clk or posedge rst) begin
    if (rst) begin
      ...
      if (acc_valid) begin
        round_reg <= round_q2_30_to_q1_15(z[0]);
        round_valid <= 1'b1;
      end else begin
        round_valid <= 1'b0;
      end
